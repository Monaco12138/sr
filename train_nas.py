...
def train(train_loader, model, optimizer):
    ...
    for batch in tqdm(train_loader, leave=False, desc='train'):
                                        # dataset 返回的是一个字典，dataloader 会将每个key对应的val叠起来，
                                        #   即每个batch[key] : [B,value]
        for k, v in batch.items():
            batch[k] = v.cuda()

        inp = (batch['inp'] - inp_sub) / inp_div            # inp: [batch, C, H, W], H = W = inp_size
        
        inp_kwargs = { 'x': inp }                           # 输入参数可根据具体的模型输入调整
